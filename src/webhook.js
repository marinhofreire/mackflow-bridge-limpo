...
export async function saveClientConfig(kv, phone, config) {
  ...
  async function sendTextToCustomer(cfg, number, text) {
    ...
    let aiReply = null;
    let sentMessage = { ok: false, reason: "no_text_message_in_payload" };

    // Busca a chave OpenAI do tenant
    const openaiKey = tenant.openaiKey || tenant.openAIKey || tenant.openai_key;
    if (messageText) {
